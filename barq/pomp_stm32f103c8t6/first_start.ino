...
void first_start() {
  if (sim_config == 0) {
    ...
    if (start_config_count == 1 && start_config_timer >= 7) {  //quectel off
    ...
    if (start_config_count == 4 && start_config_timer < 7) {  //check simcard(simcard_ok)
...
void first_config() {
  ...
  if (start_config_count == 3) {  //check sim_card
    ...
    if (sub1 == "+COPS") {
      ...
      sub1 = sim_data.substring(12, 17);  //+COPS: 0,0,"43235"
      if (sub1 == "43235") {              //simcard hamrah aval
        Serial2.println("Sim card IRAnCel");
        simcard_model = 1;
        start_config_count = 4;
        // Serial1.println("AT+CUSD=1,\"*555*4*3*2#\"");
        // delay(2000);
        // Serial1.println("AT+CUSD=1,\"#21#\"");
        // delay(2000);
      }
